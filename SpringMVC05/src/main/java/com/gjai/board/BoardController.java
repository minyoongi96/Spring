...
public class BoardController {
	...
	public String boardList(Criteria cri, Model model, HttpSession session){
	    ...
	    pageMaker.setTotalCount(boardService.boardCount(cri));
	    session.setAttribute("pageMaker", pageMaker);	// 페이지 정보 유지 위해 세션에 담기
	...
	public String reply(BoardVO vo) {
		...
		vo.setBoardGroup(parent.getBoardGroup());	// 답글의 그룹번호를 부모글로 지정
		vo.setBoardSequence(parent.getBoardSequence() + 1);	// 답글 등록 순서 지정
		vo.setBoardLevel(parent.getBoardLevel() + 1);
		
		boardService.replyUpdate(parent);	// 원글과 답글의 순서 조정
		boardService.replyInsert(vo);		// 답글 등록
		
		return "redirect:/boardView.do?idx=" + parent.getIdx();
	}
}